    ...
    <div className="w-full text-[#111827] font-display antialiased">
      ...
      <m.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="relative w-full rounded-[28px] overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.1)]">
        ...
        <div className="relative z-10 max-w-[620px] p-7 sm:p-10 md:p-14 lg:p-16 flex flex-col items-start justify-center min-h-[400px] md:min-h-[470px]">
          ...
          <m.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1], delay: 0.50 }}>
        ...
        <div className="relative z-10 max-w-[1180px] mx-auto">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-10 md:mb-14">
            <div className="max-w-[650px]">
              <p className="text-[12px] font-[800] tracking-[0.2em] text-[#059669] uppercase mb-4">SMART ECO INFRASTRUCTURE</p>
              <h2 className="font-display text-[34px] sm:text-[46px] md:text-[54px] font-[800] leading-[1.04] tracking-[-1.8px] text-[#111827]">Infrastructure that <span className="text-[#059669]">thinks in systems.</span></h2>
              <p className="mt-5 max-w-[600px] text-[15px] sm:text-[16px] leading-[1.7] text-[#64748B] font-sans">We design connected infrastructure where energy, water, mobility and materials work together — reducing waste while improving resilience, efficiency and long-term performance.</p>
            </div>
            <div className="shrink-0 rounded-2xl bg-white border border-[#E5E7EB] px-5 py-4 shadow-sm">
              <p className="text-[10px] font-[800] tracking-[0.18em] text-[#94A3B8] uppercase">SYSTEM STATUS</p>
              <div className="flex items-center gap-2 mt-2"><span className="w-2 h-2 rounded-full bg-[#10B981] animate-pulse" /><span className="font-mono text-[12px] text-[#334155]">4 NETWORKS CONNECTED</span></div>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {SYSTEMS.map((system, index) => (
              <m.div key={system.label} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5, delay: index * 0.08 }} whileHover={{ y: -6 }} className="group rounded-[22px] bg-white border border-[#E2E8F0] p-5 sm:p-6 shadow-[0_8px_30px_rgba(15,23,42,0.04)] hover:shadow-[0_18px_45px_rgba(15,23,42,0.09)] transition-shadow duration-300">
                <div className="flex items-center justify-between"><span className="w-10 h-10 rounded-xl bg-[#ECFDF5] text-[#059669] flex items-center justify-center text-[20px] font-semibold group-hover:scale-105 transition-transform">{system.icon}</span><span className="font-mono text-[10px] tracking-[0.16em] text-[#94A3B8]">0{index + 1}</span></div>
                <p className="mt-6 text-[11px] font-[800] tracking-[0.18em] text-[#64748B]">{system.label}</p>
                <div className="flex items-end justify-between mt-1"><span className="font-display text-[36px] font-[800] tracking-[-1px] text-[#111827]">{system.value}</span><span className="text-[11px] text-[#10B981] font-semibold mb-1">ACTIVE</span></div>
                <p className="mt-1 text-[12.5px] text-[#64748B] font-sans">{system.detail}</p>
                <div className="mt-5 h-1.5 rounded-full bg-[#E2E8F0] overflow-hidden"><m.div initial={{ width: 0 }} whileInView={{ width: `${system.metric}%` }} viewport={{ once: true }} transition={{ duration: 1, delay: index * 0.12 }} className="h-full rounded-full bg-gradient-to-r from-[#10B981] to-[#34D399]" /></div>
              </m.div>
            ))}
          </div>

          <div className="mt-5 grid grid-cols-1 lg:grid-cols-[1.4fr_0.6fr] gap-4">
            <m.div whileHover={{ scale: 1.005 }} className="relative min-h-[220px] rounded-[22px] bg-[#111827] overflow-hidden p-6 sm:p-8 text-white">
              ...
              <div className="relative z-10 max-w-[520px]"><p className="text-[10px] font-[800] tracking-[0.2em] text-[#6EE7B7] uppercase">CONNECTED INFRASTRUCTURE</p><h3 className="mt-3 font-display text-[25px] sm:text-[31px] font-[750] leading-tight">One network. Multiple systems. Measurable impact.</h3><p className="mt-3 text-[13px] leading-relaxed text-white/60 font-sans">Digital monitoring, resilient engineering and sustainable planning come together to keep infrastructure adaptive from construction through operation.</p></div>
              <div className="absolute right-8 bottom-7 hidden sm:block"><div className="relative w-32 h-20"><span className="absolute left-0 top-8 w-4 h-4 rounded-full bg-[#10B981] shadow-[0_0_20px_rgba(16,185,129,0.7)]" /><span className="absolute right-0 top-0 w-3 h-3 rounded-full bg-[#60A5FA]" /><span className="absolute right-4 bottom-0 w-3 h-3 rounded-full bg-[#F59E0B]" /><svg className="absolute inset-0 w-full h-full" viewBox="0 0 128 80" fill="none"><path d="M8 40L104 7M8 40L100 70" stroke="rgba(255,255,255,.25)" strokeDasharray="4 5" /></svg></div></div>
            </m.div>
            <m.div whileHover={{ y: -4 }} className="rounded-[22px] bg-white border border-[#E2E8F0] p-6 sm:p-8 flex flex-col justify-between shadow-[0_8px_30px_rgba(15,23,42,0.04)]"><div><p className="text-[10px] font-[800] tracking-[0.2em] text-[#94A3B8] uppercase">DESIGN PRINCIPLE</p><div className="mt-5 w-12 h-12 rounded-full border border-[#A7F3D0] bg-[#ECFDF5] flex items-center justify-center text-[#059669] text-xl">↻</div><h3 className="mt-5 font-display text-[22px] font-[800] text-[#111827]">Build for the next cycle.</h3><p className="mt-2 text-[13px] leading-relaxed text-[#64748B] font-sans">Plan beyond the handover with systems designed for maintenance, adaptation and continuous improvement.</p></div><div className="mt-7 flex items-center gap-2 text-[12px] font-semibold text-[#059669]">RESILIENT BY DESIGN <ArrowRight className="w-4 h-4" /></div></m.div>
          </div>
        </div>
      </section>
