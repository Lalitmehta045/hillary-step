...
export const FooterWave = React.memo(function FooterWave({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    /* ── nodes (32 crisp particles) ── */
    const nodeCount = 32;
    const nodes: WaveNode[] = Array.from({ length: nodeCount }, () => ({
      t: Math.random(),
      amp: (Math.random() - 0.5) * 320,
      phase: Math.random() * Math.PI * 2,
      speed: 0.006 + Math.random() * 0.012,
      r: Math.random() < 0.2 ? 3.5 + Math.random() * 2.5 : 1 + Math.random() * 2.0,
    }));
    ...
    const draw = (now: number) => {
      ...
      if (cachedGrad) {
        ...
        for (let b = 0; b < bands; b++) {
          const off = (b - bands / 2) * 11.0;
          ctx.beginPath();
          for (let i = 0; i <= ptsPerBand; i++) {
            const px = pxArray[i]!;
            const y =
              baseWave[i]! +
              Math.sin(px * 15 + time * 0.9 + b * 0.6) * 7 +
              off * (0.35 + px * 0.75) +
              Math.sin(px * 11 + time * 0.9 + b * 0.7) * 4;
      ...
      /* direct pair distance check for 32 particles (528 pairs, <0.02ms execution) */
      ...
      /* draw particles */
      for (let i = 0; i < nodeCount; i++) {
        const c = colorAt(ptsPx[i]!);
        ctx.beginPath();
        ctx.arc(ptsX[i]!, ptsY[i]!, ptsR[i]!, 0, Math.PI * 2);
