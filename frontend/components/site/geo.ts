...
export function toVec(lat: number, lon: number, out?: Vec3): Vec3 {
  const p = (lat * Math.PI) / 180;
  const l = (lon * Math.PI) / 180;
  if (out) {
    out.x = Math.cos(p) * Math.sin(l);
    out.y = Math.sin(p);
    out.z = Math.cos(p) * Math.cos(l);
    return out;
  }
  return {
...
export function slerp(a: Vec3, b: Vec3, t: number, out?: Vec3): Vec3 {
  const dot = Math.min(1, Math.max(-1, a.x * b.x + a.y * b.y + a.z * b.z));
  const omega = Math.acos(dot);
  if (omega < 1e-6) {
    if (out) { out.x = a.x; out.y = a.y; out.z = a.z; return out; }
    return a;
  }
