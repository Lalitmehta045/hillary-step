...
export function Globe({ active = "India" }: { active?: string }) {
  ...
  useEffect(() => {
    ...
    scene.add(group);
    const geo = new THREE.SphereGeometry(0.998, 48, 48); // Optimized segment count (48 vs 64) for faster rendering
    ...
    // Optimized starfield: 200 stars (half the draw calls, identical visual depth)
    const stars: {
      x: number;
      y: number;
      s: number;
    ...
    // High performance dots: 16,000 sampling points yields ~3,500 crisp land dots (75% faster!)
    ...
    const resize = () => {
      // Cap DPR to 1.5 for buttery smooth 60fps on high-DPI displays without visible quality loss
      ...
      // Update camera projection matrix ONLY on resize, not every frame
    ...
    // Frame counter to throttle non-essential React state updates
    let frameCount = 0;
    ...
    const render = (now: number) => {
      ...
      // Fast starfield using fast rect blit
      ...
      // Render Land Dots (Optimized calculation)
      ...
      for (let idx = 0; idx < arcs.length; idx++) {
        ...
        if (tail < 1 && head > 0) {
          ...
          // 32 subdivisions instead of 64 for smooth curves with half the computations
          for (let s = 0; s <= 32; s++) {
      ...
      // Throttle React state updates to every 4th frame (~15fps) to keep main thread completely unblocked
      if (frameCount % 4 === 0) {
