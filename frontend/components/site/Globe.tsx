...
export function Globe({ active = "India" }: { active?: string }) {
  ...
  useEffect(() => {
    ...
    const earthMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // Keep the land/cloud texture detailed while giving sunlit oceans the richer
          // electric-deep-blue appearance of the reference satellite image.
          float blueDominance = daySample.b - max(daySample.r, daySample.g) * 0.72;
          float oceanMask = smoothstep(0.025, 0.16, blueDominance);
          vec3 oceanColor = daySample.rgb;
          oceanColor = mix(
            oceanColor,
            oceanColor * vec3(0.82, 1.10, 1.42) + vec3(0.0, 0.025, 0.07),
            oceanMask * 0.9
          );

          float daylight = max(0.06, sunDot * 0.95 + 0.12);
          vec3 dayColor = oceanColor * daylight * 1.48;

          // Preserve the dark night side and its city-light layer.
    ...
    const frame = (now: number) => {
      ...
      const edgeVignette = ctx.createRadialGradient(cx, cy, R * 0.82, cx, cy, R);
      ...
      for (let i = 0; i < n; i++) {
        ...
        const rSize = Math.max(0.45, 0.8 * p.s * (R / 620));

        if (sunDot < 0.0) {
          opacity *= Math.max(0, 1.0 - (-sunDot) * 4.0);
        } else {
          opacity *= Math.max(0.25, sunIntensity);
        }
      ...
      arcs.forEach((arc, idx) => {
        ...
        const ring = (v: Vec3, alpha: number) => {
          ...
          ctx.strokeStyle = `${arc.def.hue}${Math.round(alpha * 255).toString(16).padStart(2, "0")}`;
          ctx.lineWidth = Math.max(1.2, R / 520);
          ctx.stroke();
          ctx.beginPath();
          ctx.arc(pr.x, pr.y, rr * 0.36, 0, Math.PI * 2);
          ctx.fillStyle = `${arc.def.hue}${Math.round(alpha * 255).toString(16).padStart(2, "0")}`;
          ctx.fill();
          return pr;
        };
        ...
        if (dest && headProg >= 0.9) {
          nextLabels.push({ id: idx, x: dest.x, y: dest.y, o: Math.max(0, Math.min(1, destAlpha)), def: arc.def.label });
        }
      });
      ...
      for (let i = 0; i < nextLabels.length; i++) {
        for (let j = 0; j < i; j++) {
          ...
          if (Math.abs(a.x - b.x) < 160 && Math.abs(a.y - b.y) < 42) b.y = a.y + 42;
        }
      }
    ...
    <div ref={wrap} className={`relative h-full w-full overflow-hidden transition-colors duration-700 bg-slate-950`}>
      ...
      <div className="pointer-events-none absolute inset-0 z-20">
        {labels.map((l) => (
          ...
          >
            <span className="flex h-6 w-6 items-center justify-center rounded-md text-[11px] text-primary-foreground" style={{ backgroundColor: l.def.tint }}>
              {l.def.glyph}
            </span>
            <span className="text-[13px] font-semibold text-slate-100">{l.def.city}{l.def.country ? "," : ""}</span>
            {l.def.country && <span className="text-[13px] text-slate-400">{l.def.country}</span>}
          </div>
        ))}
      </div>
    </div>
