...
function arcPath(i, total) {
  ...
  const x0 = Number((C + R * Math.cos(rad(a0))).toFixed(2));
  const y0 = Number((C + R * Math.sin(rad(a0))).toFixed(2));
  const x1 = Number((C + R * Math.cos(rad(a1))).toFixed(2));
  const y1 = Number((C + R * Math.sin(rad(a1))).toFixed(2));
...
function nodePos(i, total) {
  ...
  return {
    x: Number((C + R * Math.cos(a)).toFixed(2)),
    y: Number((C + R * Math.sin(a)).toFixed(2)),
  };
}
        ...
        <svg viewBox="0 0 560 560" className="h-full w-full" role="img" aria-label="Intelligence loop: sense, understand, predict, optimize, act, learn">
          ...
          <g className="loop-ring-spin">
            {Array.from({ length: 36 }).map((_, i) => {
              ...
              const cx = Number((C + (R + 26) * Math.cos(a)).toFixed(2));
              const cy = Number((C + (R + 26) * Math.sin(a)).toFixed(2));
              return (
                <circle
                  key={i}
                  cx={cx}
